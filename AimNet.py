...
MARGIN = 1.0
EMBEDDING_SIZE = 128
NUM_CLASSES_SPLITTER = 1000  # Number of users used for training
NUM_CLASSES_CLASSIFIER = 1000
...
EMBEDDER_IN_PATH = './models/aimnet_embedding_model.pth'
...
FREEZE_BASE = False  # Set 'True' to freeze the base classifier when training the embedding model.
DO_SPLIT_TEST = True  # Set 'True' when training and testing on the same dataset.
...
def main():
    if DO_PREPROCESSING:
        ...
        if DO_SPLIT_TEST:
            train_sequences, train_labels, test_sequences, test_labels = (
                user_based_train_test_split(sequences, labels, NUM_CLASSES_SPLITTER))
    ...
    if MODE == 'train_classifier_model':
        ...
        classification_model = LSTMClassifier(input_size=input_size, output_size=NUM_CLASSES_CLASSIFIER)
        ...
        #embedding_model.load_state_dict(torch.load(EMBEDDER_IN_PATH))  # For finetuning
        ...
        if DO_PREPROCESSING:
            classification_model = LSTMClassifier(input_size, NUM_CLASSES_CLASSIFIER)
        ...
        num_test_users = len(np.unique(labels))
        print(f"Evaluating embedding model on the {num_test_users} hold-out test users")
